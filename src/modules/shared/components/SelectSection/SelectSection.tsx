...
import { getCurrentColors } from '../../servises/createVar';
...
export const SelectSection: React.FC<SelectSectionProps> = ({ content }) => {
  const { category } = useParams();
  const navigate = useNavigate();
  const { setSelectedProduct } = useContext(ProductListContext);
  ...
  const getNewOption = (
    ...
    if (optionType === 'color') {
      ...
      setSelectedProduct(productChanged);
    }
